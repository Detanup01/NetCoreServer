...
public class SslSession(SslServer server) : IDisposable
{
    ...
    internal void Connect(Socket socket)
    {
        ...
        try
        {
            ...
            _sslStream = new(new NetworkStream(Socket, false), false, Server.Context.CertificateValidationCallback);
            ...
            {
                ServerCertificate = Server.Context.Certificate,
                ClientCertificateRequired = Server.Context.ClientCertificateRequired,
                CertificateRevocationCheckMode = Server.Context.RevocationMode,
                EnabledSslProtocols = Server.Context.Protocols,
                ServerCertificateContext = Server.Context.CertificateContext,
                ServerCertificateSelectionCallback = Server.Context.CertificateContext == null && Server.Context.Certificate == null ? Server.Context.CertificateSelectionCallback : null,
            };

            // Begin the SSL handshake
            TaskToAsyncResult.Begin(_sslStream.AuthenticateAsServerAsync(sslOptions, CancellationToken.None)!, ProcessHandshake, _sslStreamId);
        }
        catch (Exception ex)
        {
            SendError(ex, SocketError.NotConnected);
            Disconnect();
        }
    }
    ...
    public virtual long Send(ReadOnlySpan<byte> buffer)
    {
        ...
        {
            SendError(ex, SocketError.OperationAborted);
            Disconnect();
            return 0;
        }
    }
    ...
    /// <param name="ex">Exception in the socket</param>
    /// <param name="error">Socket error code</param>
    private void SendError(Exception ex, SocketError error)
    {
