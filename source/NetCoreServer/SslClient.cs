...
public class SslClient : IDisposable
{
    ...
    public virtual bool Connect()
    {
        ...
        catch (SocketException ex)
        {
            ...
            SendError(ex, ex.SocketErrorCode);
        ...
        try
        {
            ...
            IsHandshaking = true;
            if (Context.Certificate != null && Context.Certificates == null)
                Context.Certificates = [.. new[] { Context.Certificate as X509Certificate2 }];
            SslClientAuthenticationOptions options = new()
            {
                TargetHost = Address,
                ClientCertificates = Context.Certificates,
                EnabledSslProtocols = Context.Protocols,
                CertificateRevocationCheckMode = Context.RevocationMode,
                ClientCertificateContext = Context.CertificateContext,
            };
    ...
    public virtual long Receive(byte[] buffer, long offset, long size)
    {
        ...
        {
            SendError(null, SocketError.OperationAborted);
            Disconnect();
            return 0;
        }
    }
    ...
    private void ProcessConnect(SocketAsyncEventArgs e)
    {
        ...
        if (e.SocketError == SocketError.Success)
        {
            ...
            try
            {
                ...
                TaskToAsyncResult.Begin(_sslStream.AuthenticateAsClientAsync(options, CancellationToken.None)!, ProcessHandshake, _sslStreamId);
            }
            catch (Exception ex)
            {
                SendError(ex, SocketError.NotConnected);
