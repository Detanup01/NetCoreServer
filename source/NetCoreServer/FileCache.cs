...
public class FileCache : IDisposable
{
    ...
    public bool InsertPath(string path, string prefix = "/", string filter = "*.*", TimeSpan timeout = new TimeSpan(), InsertHandler handler = null)
    {
        ...
        using (new WriteLock(_lockEx))
        {
            ...
            _pathsByKey.Add(path, new(this, prefix, path, filter, handler, timeout));
    ...
    private class FileCacheEntry
    {
        ...
        private static void OnCreated(object _, FileSystemEventArgs e, FileCache cache, FileCacheEntry entry)
        {
        ...
        private static void OnChanged(object _, FileSystemEventArgs e, FileCache cache, FileCacheEntry entry)
        {
        ...
        private static void OnDeleted(object _, FileSystemEventArgs e, FileCache cache, FileCacheEntry entry)
        {
            var key = e.FullPath.Replace('\\', '/').Replace(entry._path, entry._prefix).RemoveSuffix('/');
        ...
        private static void OnRenamed(object _, RenamedEventArgs e, FileCache cache, FileCacheEntry entry)
        {
